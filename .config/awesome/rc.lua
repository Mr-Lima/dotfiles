...
screen.connect_signal("arrange", function (s)
    ...
    for _, c in pairs(s.clients) do
        if only_one and not c.floating or c.maximized or c.maximized then
            c.border_width = 2
        else
...
globalkeys = my_table.join(
    ...
    awful.key({ modkey }, "w", function () awful.util.spawn( browser3 ) end,
    ...
    awful.key({ modkey }, "r", function () awful.util.spawn( "urxvt" .. " -e 'spt'", { tag = awful.util.tagnames[5], screen = 1} ) end,
        {description = "spotify-tui", group = "super"}),
    awful.key({ modkey }, "t", function () awful.util.spawn( terminal2 ) end,
        {description = "terminal2", group = "super"}),
      ...
      function ()
        awful.prompt.run {
                    ...
                    exe_callback = function(input)
                          awful.util.spawn( "konsole --workdir " .. (input and input or "$(pwd)"), { tag = awful.util.tagnames[1], screen = 1} )
                          awful.util.spawn( "konsole --workdir " .. (input and input or "$(pwd)"), { tag = awful.util.tagnames[1], screen = 1} )
                          awful.util.spawn( "konsole --workdir " .. (input and input or "$(pwd)"), { tag = awful.util.tagnames[1], screen = 1, focus = true} )
                    end,
                    history_path = awful.util.get_cache_dir() .. "/history_eval"
                  }
        -- awful.util.spawn( "konsole --workdir $(pwd)", { tag = awful.util.tagnames[1], screen = 1} )
        -- awful.util.spawn( "konsole --workdir $(pwd)", { tag = awful.util.tagnames[1], screen = 1} )
        -- awful.util.spawn( "konsole --workdir $(pwd)", { tag = awful.util.tagnames[1], screen = 1, focus = true} )
      end,
      {description = "Open Ide", group = "super"}
    ),
